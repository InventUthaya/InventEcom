...
export const TabletSlider = ({ title }: { title: string, }) => {
        ...
        CategoryService.getCategoryList()
            .then((res: any) => {
                if (res.status === 200 && res.data) {
                    ...
                    const tabletCategory = allCategories.find(
                        (category: { Name?: string }) => category.Name?.toLowerCase() === "tablet"
                    );
                    if (tabletCategory) {
                        setTabletId(tabletCategory.EncryptedId);
                    }
                }
            })
            .catch((e: string) => {
