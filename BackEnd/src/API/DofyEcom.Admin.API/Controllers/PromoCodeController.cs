...
namespace DofyEcom.Admin.API.Controllers
{
    ...
    public class PromoCodeController : BaseController<IPromoCodeModel, ViewEntities.PromoCode>
    {
        ...
        //[HttpGet("GetSKUsByPromo")]
        //public async Task<IActionResult> GetSKUsByPromo(int promoId)
        //{
        //    var skus = await promoModel.GetSKUsByPromoAsync(promoId);
        //    return Ok(new { status = 200, message = "Success", data = skus });
        //}

        //[HttpPost("AssignSKUsToPromo")]
        //public async Task<IActionResult> AssignSKUsToPromo([FromBody] dynamic req)
        //{
        //    var promoId = (int)req.PromoId;
        //    IEnumerable<int> skuIds = ((IEnumerable<object>)req.SkuIds).Select(x => (int)x);
        //    var ok = await promoModel.AssignSKUsToPromoAsync(promoId, skuIds);
        //    return Ok(new { status = ok ? 200 : 400, message = ok ? "Assigned" : "Failed" });
        //}
    }
}
